...
po::options_description createGeneralOptionsDescription(Context &context,
  ...
  options.add_options()(
      "seed", po::value<int>(&context.partition.seed)->value_name("<int>"),
      ...
      "part-file",
      po::value<std::string>(&context.partition.input_partition_filename)
          ->value_name("<string>"),
      "Input Partition filename. The input partition is then refined using "
      "direct k-way V-cycles.")(
      "cmaxnet",
      po::value<HyperedgeID>(&context.partition.hyperedge_size_threshold)
          ->value_name("<uint32_t>"),
      "Hyperedges larger than cmaxnet are ignored during partitioning "
      "process.")(
      "vcycles",
      po::value<uint32_t>(&context.partition.global_search_iterations)
          ->value_name("<uint32_t>"),
      "# V-cycle iterations for direct k-way partitioning")(
      "use-individual-part-weights",
      po::value<bool>(&context.partition.use_individual_part_weights)
          ->value_name("<bool>"),
      "# Use individual part weights specified with --partweights= option")(
      "part-weights",
      po::value<std::vector<HypernodeWeight>>(
...
createFlowRefinementOptionsDescription(Context &context, const int num_columns,
  ...
  po::options_description options(
      (initial_partitioning ? "Initial Partitioning Flow Refinement Options"
                            : "Flow Refinement Options"),
      num_columns);

  options.add_options()(
      (initial_partitioning ? "i-r-flow-execution-policy"
                            : "r-flow-execution-policy"),
      po::value<std::string>()
          ->value_name("<string>")
          ->notifier([&context,
                      initial_partitioning](const std::string &ftype) {
            if (initial_partitioning) {
              context.initial_partitioning.local_search.flow.execution_policy =
                  kahypar::flowExecutionPolicyFromString(ftype);
            } else {
      ...
      " - constant    : Execute flows in each level i with i = beta * j (j "
      "\\in {1,2,...})\n"
      " - exponential : Execute flows in each level i with i = 2^j (j \\in "
      "{1,2,...})\n"
      " - multilevel  : Execute flows in each level i with i = |V|/2^j (j \\in "
      "{1,2,...})\n"
      "(default: exponential)")(
...
po::options_description createHyperFlowCutterRefinementOptionsDescription(
    Context &context, const int num_columns, const bool initial_partitioning) {
  po::options_description options(("HyperFlowCutter Refinement Options"),
                                  num_columns);
  options.add_options()(
      (initial_partitioning ? "i-r-hfc-size-constraint"
                            : "r-hfc-size-constraint"),
      po::value<std::string>()
          ->value_name("<string>")
          ->notifier([&context,
                      initial_partitioning](const std::string &ftype) {
            FlowHypergraphSizeConstraint sc =
                FlowHypergraphSizeConstraint::part_weight_fraction; // = "pw"
            if (ftype == "mpw") {
              sc = FlowHypergraphSizeConstraint::max_part_weight_fraction;
            } else if (ftype == "mf-style") {
              sc = FlowHypergraphSizeConstraint::
                  scaled_max_part_weight_fraction_minus_opposite_side;
              if (initial_partitioning) {
                if (context.initial_partitioning.local_search.hyperflowcutter
                        .snapshot_scaling < 1.0) {
                  context.initial_partitioning.local_search.hyperflowcutter
                      .snapshot_scaling = 16.0;
                }
              } else {
                if (context.local_search.hyperflowcutter.snapshot_scaling <
                    1.0) {
              ...
              throw std::runtime_error(
                  "Unknown option flow hypergraph size constraint option");
            }
            if (initial_partitioning) {
              context.initial_partitioning.local_search.hyperflowcutter
                  .flowhypergraph_size_constraint = sc;
            } else {
              context.local_search.hyperflowcutter
                  .flowhypergraph_size_constraint = sc;
            }
          }),
      "Size Constraints:\n"
      ...
      " - pw             : |N_0| <= |V_0| * alpha \n"
      " - mf-style       : |N_0| <= (1 + alpha * epsilon) * "
      "perfect_part_weight[b0] - |V_1|. Same as KaHyPar-MF. Default scaling is "
      "16. \n"
      "(default: pw)")(
      ...
      po::value<double>(
          (initial_partitioning
               ? &context.initial_partitioning.local_search.hyperflowcutter
                      .snapshot_scaling
               : &context.local_search.hyperflowcutter.snapshot_scaling))
          ->value_name("<double>"),
      "Scaling parameter for flow hypergraph sizes for HFC refinement. see "
      "size constraints for semantics.\n"
      "(default: 0.2)")(
      (initial_partitioning ? "i-r-hfc-distance-based-piercing"
                            : "r-hfc-distance-based-piercing"),
      po::value<bool>(
          (initial_partitioning
               ? &context.initial_partitioning.local_search.hyperflowcutter
                      .use_distances_from_cut
               : &context.local_search.hyperflowcutter.use_distances_from_cut))
          ->value_name("<bool>"),
      ...
      po::value<bool>(
          (initial_partitioning
               ? &context.initial_partitioning.local_search.hyperflowcutter
                      .most_balanced_cut
               : &context.local_search.hyperflowcutter.most_balanced_cut))
          ->value_name("<bool>"),
...
po::options_description
createCoarseningOptionsDescription(Context &context, const int num_columns,
                                   const bool initial_partitioning) {
  po::options_description options(
      (initial_partitioning ? "Initial Partitioning Coarsening Options"
                            : "Coarsening Options"),
      num_columns);
  options.add_options()(
              ...
              [&context, initial_partitioning](const std::string &ctype) {
                if (initial_partitioning) {
                  context.initial_partitioning.coarsening.algorithm =
                      kahypar::coarseningAlgorithmFromString(ctype);
                } else {
      ...
      po::value<double>(
          (initial_partitioning
               ? &context.initial_partitioning.coarsening
      ...
      po::value<std::string>()
          ->value_name("<string>")
          ->notifier([&context,
                      initial_partitioning](const std::string &rating_score) {
            if (initial_partitioning) {
      ...
      "- heavy_edge")(
      (initial_partitioning ? "i-c-rating-use-communities"
      ...
      "Use community information during rating. If "
      "c-rating-use-communities=true ,\n"
      "only neighbors belonging to the same community will be considered as "
      "contraction partner.")(
      (initial_partitioning ? "i-c-rating-heavy_node_penalty"
      ...
      po::value<std::string>()
          ...
          ->notifier(
              [&context, initial_partitioning](const std::string &penalty) {
                if (initial_partitioning) {
                  context.initial_partitioning.coarsening.rating
                      .heavy_node_penalty_policy =
      ...
      "- multiplicative\n"
      "- no_penalty")(
      (initial_partitioning ? "i-c-rating-acceptance-criterion"
                            : "c-rating-acceptance-criterion"),
      po::value<std::string>()
          ->value_name("<string>")
          ->notifier([&context, initial_partitioning](const std::string &crit) {
            if (initial_partitioning) {
      ...
      "Acceptance/Tiebreaking criterion for contraction partners having the "
      "same score:\n"
      "- best\n"
      "- best_prefer_unmatched")(
      (initial_partitioning ? "i-c-fixed-vertex-acceptance-criterion"
      ...
      po::value<std::string>()
          ...
          ->notifier([&context, initial_partitioning](const std::string &crit) {
            if (initial_partitioning) {
              context.initial_partitioning.coarsening.rating
                  .fixed_vertex_acceptance_policy =
      ...
      "- fixed_vertex_allowed : Allows (free, free), (fixed, free), and "
      "(fixed, fixed) \n"
...
createRefinementOptionsDescription(Context &context, const int num_columns,
                                   const bool initial_partitioning) {
  po::options_description options(
      (initial_partitioning ? "Initial Partitioning Refinement Options"
                            : "Refinement Options"),
      num_columns);
  options.add_options()(
              ...
              [&context, initial_partitioning](const std::string &rtype) {
                if (initial_partitioning) {
                  context.initial_partitioning.local_search.algorithm =
                      kahypar::refinementAlgorithmFromString(rtype);
                } else {
      ...
      " - twoway_fm                    : 2-way FM algorithm         (recursive "
      "bisection : cut & km1)\n"
      " - twoway_hyperflow_cutter      : 2-way HyperFlowCutter      (recursive "
      "bisection : cut & km1)\n"
      " - twoway_fm_hyperflow_cutter   : 2-way FM + HyperFlowCutter (recursive "
      "bisection : cut & km1)\n"
      " - kway_fm                      : k-way FM algorithm         (direct "
      "k-way        : cut)\n"
      " - kway_fm_hyperflow_cutter     : k-way FM + HyperFlowCutter (direct "
      "k-way        : cut)\n"
      " - kway_fm_km1                  : k-way FM algorithm         (direct "
      "k-way        : km1)\n"
      " - kway_fm_hyperflow_cutter_km1 : k-way FM + HyperFlowCutter (direct "
      "k-way        : km1)\n"
      " - kway_hyperflow_cutter        : k-way HyperFlowCutter      (direct "
      "k-way        : cut & km1)\n")(
      (initial_partitioning ? "i-r-runs" : "r-runs"),
      po::value<int>(
          (initial_partitioning
               ? &context.initial_partitioning.local_search.iterations_per_level
               : &context.local_search.iterations_per_level))
          ->value_name("<int>")
          ->notifier([&context, initial_partitioning](const int) {
            if (initial_partitioning) {
              if (context.initial_partitioning.local_search
              ...
              [&context, initial_partitioning](const std::string &stopfm) {
                if (initial_partitioning) {
                  context.initial_partitioning.local_search.fm.stopping_rule =
                      kahypar::stoppingRuleFromString(stopfm);
                } else {
      ...
      po::value<uint32_t>(
          (initial_partitioning
               ? &context.initial_partitioning.local_search.fm
                      .max_number_of_fruitless_moves
               : &context.local_search.fm.max_number_of_fruitless_moves))
          ->value_name("<uint32_t>"),
      "Max. # fruitless moves before stopping local search using simple "
      "stopping rule")(
      (initial_partitioning ? "i-r-fm-stop-alpha" : "r-fm-stop-alpha"),
      po::value<double>(
          (initial_partitioning
               ? &context.initial_partitioning.local_search.fm
                      .adaptive_stopping_alpha
               : &context.local_search.fm.adaptive_stopping_alpha))
          ->value_name("<double>"),
      "Parameter alpha for adaptive stopping rule \n"
      "(infinity: -1)");
  options.add(createFlowRefinementOptionsDescription(context, num_columns,
                                                     initial_partitioning));
  options.add(createHyperFlowCutterRefinementOptionsDescription(
      context, num_columns, initial_partitioning));
  return options;
}

po::options_description
createInitialPartitioningOptionsDescription(Context &context,
                                            const int num_columns) {
  po::options_description options("Initial Partitioning Options", num_columns);
  options.add_options()("i-mode",
                        po::value<std::string>()
                            ->value_name("<string>")
                            ->notifier([&](const std::string &ip_mode) {
                              context.initial_partitioning.mode =
                                  kahypar::modeFromString(ip_mode);
                            }),
                        "IP mode: \n"
                        " - (recursive) bisection  \n"
                        " - (direct) k-way")(
      ...
      po::value<std::string>()
          ->value_name("<string>")
          ->notifier([&](const std::string &ip_technique) {
      ...
      po::value<std::string>()
          ->value_name("<string>")
          ->notifier([&](const std::string &ip_algo) {
      ...
      po::value<std::string>()
          ->value_name("<string>")
          ->notifier([&](const std::string &ip_bp_algo) {
      ...
      "(default: worst_fit)")(
      "i-bp-early-restart",
      po::value<bool>(&context.initial_partitioning.enable_early_restart)
          ->value_name("<bool>"),
      "Enable early restart with prepacking of current bisection if infeasible"
      "(default: false)")(
      "i-bp-late-restart",
      po::value<bool>(&context.initial_partitioning.enable_late_restart)
          ->value_name("<bool>"),
      "Enable late restart with prepacking of bisections, i.e. if the "
      "resulting partition is imbalanced"
      "(default: false)")(
      "i-bp-heuristic-prepacking",
      po::value<bool>(&context.initial_partitioning.use_heuristic_prepacking)
          ->value_name("<bool>"),
      "Try a heuristic prepacking berfore using the one with balance guarantees"
      "(default: true)")(
      "i-runs",
      po::value<uint32_t>(&context.initial_partitioning.nruns)
          ->value_name("<uint32_t>"),
...
po::options_description
createPreprocessingOptionsDescription(Context &context, const int num_columns) {
  po::options_description options("Preprocessing Options", num_columns);
  options.add_options()(
      "p-enable-deduplication",
      po::value<bool>(&context.preprocessing.enable_deduplication)
          ->value_name("<bool>"),
      "Remove identical vertices and parallel nets before partitioning")(
      "p-use-sparsifier",
      po::value<bool>(&context.preprocessing.enable_min_hash_sparsifier)
          ->value_name("<bool>"),
      ...
      po::value<uint32_t>(&context.preprocessing.min_hash_sparsifier
                               .combined_num_hash_functions)
          ->value_name("<int>"),
      "Number of combined hash functions")(
      "p-detect-communities",
      po::value<bool>(&context.preprocessing.enable_community_detection)
          ->value_name("<bool>"),
      "Using louvain community detection for coarsening")(
      "p-detect-communities-in-ip",
      po::value<bool>(&context.preprocessing.community_detection
                           .enable_in_initial_partitioning)
          ->value_name("<bool>"),
      "Using louvain community detection for coarsening during initial "
      "partitioning")(
      ...
      "Minimum improvement of quality during a louvain pass which leads to "
      "further passes")(
      "p-louvain-edge-weight",
      po::value<std::string>()
          ->value_name("<string>")
          ->notifier([&](const std::string &ptype) {
            context.preprocessing.community_detection.edge_weight =
                kahypar::edgeWeightFromString(ptype);
          }),
      "Weights:\n"
      " - hybrid \n"
      " - uniform\n"
      ...
      po::value<bool>(
          ...
          ->value_name("<bool>"),
      "Reuse the community structure identified in the first bisection for all "
      "other bisections.");
  return options;
}

po::options_description
createEvolutionaryOptionsDescription(Context &context, const int num_columns) {
  po::options_description evolutionary_options("Evolutionary Options",
                                               num_columns);
  evolutionary_options.add_options()(
      ...
      po::value<size_t>()
          ->value_name("<size_t>")
          ->notifier([&](const size_t &pop_size) {
      ...
      po::value<std::string>()
          ->value_name("<string>")
          ->notifier([&](const std::string &replace_strat) {
            context.evolutionary.replace_strategy =
                kahypar::replaceStrategyFromString(replace_strat);
          }),
      "Replacement Strategy for Population Management\n"
      "- worst: new partitions replace the current worst partition in the "
      "population\n"
      "- diverse: new partitions replace the most similar partition based on "
      "cut difference\n"
      "- strong-diverse: new partitions replace the most similar partition "
      "based on connectivity difference\n"
      "(for diverse/strong-diverse better partitions are not considered for "
      "replacement)\n"
      "(default: strong-diverse)")(
      "combine-strategy",
      po::value<std::string>()
          ->value_name("<string>")
          ->notifier([&](const std::string &combine_strat) {
            context.evolutionary.combine_strategy =
                kahypar::combineStrategyFromString(combine_strat);
          }),
      "Combine Strategy to be used for a regular combine operation\n"
      "- basic: takes two partitions and contracts nodes u & v only if they "
      "are in the same block for both partitions \n"
      "- with-edge-frequency: similar to basic, but the edge frequency "
      "information of the best \"edge_frequency_amount\" partitions is added "
      "top the rating\n"
      "- edge-frequency: creating a new partition under consideration of the "
      "\"edge_frequency_amount\" best partitions for edge frequency\n"
      "(default: basic)")(
      "mutate-strategy",
      po::value<std::string>()
          ->value_name("<string>")
          ->notifier([&](const std::string &mutate_strat) {
            context.evolutionary.mutate_strategy =
                kahypar::mutateStrategyFromString(mutate_strat);
          }),
      "Mutation Strategy for the mutation operation \n"
      "- new-initial-partitioning-vcycle: coarsening of a partition with "
      "completely new initial partitioning\n"
      ...
      "default: 0.15)")("random-combine",
                        po::value<bool>()->value_name("<bool>")->notifier(
                            [&](const bool &random_combine) {
                              context.evolutionary.random_combine_strategy =
                                  random_combine;
                            }),
      ...
      "default: 0.5)")("edge-frequency-chance",
                       po::value<float>()->value_name("<float>")->notifier(
                           [&](const float &edge_chance) {
                             context.evolutionary.edge_frequency_chance =
                                 edge_chance;
                           }),
                       "The Chance of a mutation being selected as operation\n"
                       "default: 0.5)");
...
po::options_description createGenericOptionsDescription(Context &context,
  ...
  generic_options.add_options()("help", "show help message")(
      "verbose,v",
      po::value<bool>(&context.partition.verbose_output)->value_name("<bool>"),
      "Verbose main partitioning output")(
      "vip",
      po::value<bool>(&context.initial_partitioning.verbose_output)
          ->value_name("<bool>"),
      "Verbose initial partitioning output")(
      "quiet,q",
      po::value<bool>(&context.partition.quiet_mode)->value_name("<bool>"),
      "Quiet Mode: Completely suppress console output")(
      "time-limit",
      po::value<int>(&context.partition.time_limit)->value_name("<int>"),
      "Sets a time limit in seconds. default: disabled. "
      "We stop refinement once a large part (default 99%) is exceeded."
      "It is never triggered before an initial partition is available, so you "
      "should still provide an external timeout (only now you might get a "
      "solution).")("time-limit-factor",
                    po::value<double>(&context.partition.soft_time_limit_factor)
                        ->value_name("<double>"),
                    "Controls the refinement time limit. default: 0.99")(
      "time-limit-check-frequency",
      po::value<int>(&context.partition.soft_time_limit_check_frequency)
          ->value_name("<int>"),
      "After how many uncontractions the soft time limit shall be checked. "
      "default 10000")(
      "time-limited-repeated-partitioning",
      po::value<bool>(&context.partition.time_limited_repeated_partitioning)
          ->value_name("<bool>"),
      "Use repeated partitioning with the strict time limit set using "
      "--time-limit. This also uses the soft time limit.")(
      "sp-process,s",
      po::value<bool>(&context.partition.sp_process_output)
          ->value_name("<bool>"),
      "Summarize partitioning results in RESULT line compatible with "
      "sqlplottools "
      "(https://github.com/bingmann/sqlplottools)")(
      "write-partition,w",
      po::value<bool>(&context.partition.write_partition_file)
          ->value_name("<bool>"),
      "Write output partition. Default: false");
...
void processCommandLineInput(Context &context, int argc, char *argv[]) {
  ...
  required_options.add_options()(
      ...
      po::value<std::string>(&context.partition.graph_filename)
          ->value_name("<string>")
          ->required(),
      "Hypergraph filename")("blocks,k",
                             po::value<PartitionID>(&context.partition.k)
                                 ->value_name("<int>")
                                 ->required()
                                 ->notifier([&](const PartitionID) {
                                   context.partition.rb_lower_k = 0;
                                   context.partition.rb_upper_k = 0;
                                 }),
                             "Number of blocks")(
      "epsilon,e",
      po::value<double>(&context.partition.epsilon)
          ->value_name("<double>")
          ->required(),
  ...
  po::options_description coarsening_options =
      createCoarseningOptionsDescription(context, num_columns, false);

  po::options_description ip_options =
  ...
  write_snapshot.add_options()(
      "snapshot-path",
      po::value<std::string>(
          &context.local_search.hyperflowcutter.snapshot_path)
          ->value_name("<string>"),
      "Path for flow hypergraph snapshots. Default: None (don't write them)");
  ...
  if (!file) {
    std::cerr << "Could not load context file at: " << context_path
              << std::endl;
    std::exit(-1);
  }
  ...
  context.partition.graph_partition_filename =
      context.partition.graph_filename + ".part" +
...
void parseIniToContext(Context &context, const std::string &ini_filename) {
  ...
  if (!file) {
    std::cerr << "Could not load context file at: " << ini_filename
              << std::endl;
    std::exit(-1);
  }
  ...
  if (context.partition
          .use_individual_part_weights) { // Note(Lars): This affects flow
                                          // network sizes!
    context.partition.epsilon = 0;
  }
}

void iniToContext(Context &context) {
  const int num_columns = 80;
...
#pragma GCC diagnostic ignored "-Wwrite-strings"
  ...
  // po::store(po::parse_config_file(file, ini_line_options, true), cmd_vm);
  po::store(parse_command_line(sizeof(options) / sizeof(options[0]), options,
                               ini_line_options),
            cmd_vm);
  po::notify(cmd_vm);
