...
generateSatisfiableCNFLimited()
{
    ...
    nbShow=$(echo "$nbVar * 0.8" | bc | cut -d '.' -f1)
    shufVar=$(seq 1 $nbVar | shuf | tr '\n' ' ')
    listShow=$(echo $shufVar | cut -d ' ' -f1-$nbShow)

    nbProtected=$(echo "$nbShow * 0.2" | bc | cut -d '.' -f1)
    listProtected=$(echo $listShow | cut -d ' ' -f1-$nbProtected)

   # echo "c p show ${listShow} 0" >> /tmp/test.cnf
   # echo "c p protected ${listProtected} 0" >> /tmp/test.cnf
    echo "/tmp/test.cnf"    
}
